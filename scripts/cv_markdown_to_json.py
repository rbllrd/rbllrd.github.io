...
def parse_course-staff(course-staff_dir):
    """Parse course-staff from the _course-staff directory."""
    course-staff = []
    
    if not os.path.exists(course-staff_dir):
        return course-staff
    
    for course-staff_file in sorted(glob.glob(os.path.join(course-staff_dir, "*.md"))):
        with open(course-staff_file, 'r', encoding='utf-8') as file:
            content = file.read()
        ...
        if front_matter_match:
            ...
            course-staff.append(course-staff_entry)
...
def create_cv_json(md_file, config_file, repo_root, output_file):
    ...
    # Add course-staff
    cv_json["course-staff"] = parse_course-staff(os.path.join(repo_root, "_course-staff"))
